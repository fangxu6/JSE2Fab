...
{
    public class TskMergeProcessor : ITskProcessor
    {
        ...
        public void Process(List<string> firstFiles, List<string> secondFiles, Action<string> updateStatus, ProgressBar progressBar = null)
        {
            for (int i = 0; i < firstFiles.Count; i++)
            {
                if (TskFileHelper.ValidateFiles(firstFiles[i], secondFiles[i]))
                {
                    ProcessTskMerge(firstFiles[i], secondFiles[i], updateStatus);
                }
            }
        }

...
namespace DataToExcel
{
    ...
    public class DieData
    {
        ...
        public static DieData operator +(DieData item1, DieData item2)
        {
            ...
            if ((item1.Attribute == DieCategory.MarkDie))
            {
                data.Attribute = DieCategory.MarkDie;
                return data;
            }
            if ((item1.Attribute == DieCategory.Unknow))
            {
                data.Attribute = DieCategory.Unknow;
                return data;
            }
            if ((item1.Attribute == DieCategory.SkipDie))
            {
                data.Attribute = DieCategory.SkipDie;
                return data;
            }
            if ((item1.Attribute == DieCategory.SkipDie2))
            {
                data.Attribute = DieCategory.SkipDie2;
                return data;
            }

            if ((item1.Attribute == DieCategory.PassDie || item1.Attribute == DieCategory.FailDie) 
                    && (item2.Attribute == DieCategory.MarkDie || 
                    item2.Attribute == DieCategory.Unknow ||
                    item2.Attribute == DieCategory.SkipDie ||
                    item2.Attribute == DieCategory.SkipDie2 ||
                    item2.Attribute == DieCategory.NoneDie))
            {
            ...
            //if ((item1.Attribute == DieCategory.PassDie || item1.Attribute == DieCategory.FailDie)
            //        && (item2.Attribute == DieCategory.FailDie))
            //{
            //    data.Attribute = DieCategory.FailDie;
            //    data.Bin = item2.Bin;
            //    return data;

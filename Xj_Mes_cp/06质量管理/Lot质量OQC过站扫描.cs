...
namespace Xj_Mes_cp
{
    public partial class Lot质量OQC过站扫描 : DockContent
    {
        public Lot质量OQC过站扫描()
        {
            InitializeComponent();
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            //是否已经有库存了
            DataTable dt_product = ex.Get_Data("[dbo].[hp_20220330_process_check_wms_no_select] '" + LOT_ONLY_CODE + "'");
            if (dt_product.Rows[0][0].ToString() == "0")
            {
                MessageBox.Show("流程卡未完成《出片检验》站别扫描,请确认！", "系统提示"); return;
            }



            ////是否已经过站了
            //DataTable dt_oqc = ex.Get_Data("[dbo].[hp_1116_qc_info_deal_check] '" + LOT_ONLY_CODE + "','OQC检验'");
            //if (dt_oqc.Rows[0][0].ToString()=="1")
            //{
            //    MessageBox.Show("流程卡已经过站,请确认！", "系统提示"); return;
            //}
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            ...
            //是否已经有库存了
            DataTable dt_product = ex.Get_Data("[dbo].[hp_20220330_process_check_wms_no_select] '" + only_code + "'");
            if (dt_product.Rows[0][0].ToString() == "0")
            {
                MessageBox.Show("流程卡未完成《出片检验》站别扫描,请确认！", "系统提示"); return;
            }
            ...
            DataTable dt_check = ex.Get_Data("[dbo].[hp_1116_qc_info_check_number_select]  '" + only_code + "','OQC检验','" + post_info + "'");

            if (dt_check.Rows[0][0].ToString() == "1")
            {
                MessageBox.Show("当前流程卡位号已经通过OQC", "系统提示"); return;
            }


            DataTable dt = ex.Get_Data("[dbo].[hp_1116_qc_info_insert01]  'OQC检验','" + only_code + "','" + qc_res + "','" + deal_res + "','" + do_user + "','"+post_info+"','"+post_number+"'");

...
namespace Xj_Mes_cp
{
    public partial class 业务排产打印管理新版流程卡版本 : DockContent
    {
        public 业务排产打印管理新版流程卡版本()
        {
            InitializeComponent();
        }
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            ...
            string ONLY_CODE=this.textBoxX15.Text;
            if (ONLY_CODE=="")
            {
                MessageBox.Show("请输入流程卡号", "系统提示"); return;
            }

            string sql_str = string.Format("[dbo].[hp_0915_business_info_by_onlycode_select] '{0}' ", ONLY_CODE); ;
            DataTable dt = ex.Get_Data(sql_str);

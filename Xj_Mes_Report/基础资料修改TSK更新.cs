...
{
    public partial class 基础资料修改TSK更新 : DockContent
    {
        public 基础资料修改TSK更新()
        {
            InitializeComponent();
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = db_Helper.Get_Data("[dbo].[hp_20220112_tsk_info_by_lot_select] '" + lot + "'");
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (site_bin.Tables[0].Rows.Count > 0)
                        {
                            for (int j = 0; j < site_bin.Tables[0].Rows.Count; j++)
                            {
                                String site = site_bin.Tables[0].Rows[i][0].ToString();
                                String site_zb = site_bin.Tables[0].Rows[i][3].ToString();
                                ...
                                for (int x = 0; x < dt_site_warm.Rows.Count; x++)
                                {
                                    ...
                                    {
                                        db_Helper.Sys_Exe_Data(String.Format("[dbo].[hp_cp_tsk_site_warm_insert] '{0}','{1}','{2}','{3}','{4}'", id, warm_site, site_zb, warm_site_zb, "小于"));
                                    }
                        ...
                        if (site_bin.Tables[1].Rows.Count > 0)
                        {
                            for (int bin_i = 0; bin_i < site_bin.Tables[1].Rows.Count; bin_i++)
                            {
                                ...
                                String bin = site_bin.Tables[1].Rows[i][0].ToString();
                                String bin_zb = site_bin.Tables[1].Rows[i][3].ToString();
                                    ...
                                    if (Convert.ToInt32(bin) == Convert.ToInt32(warm_bin))
                                    {
                                        switch (warm_set)
                                        {
                                            case "大于":
                                                ...
                                                {
                                                    db_Helper.Sys_Exe_Data(String.Format("[dbo].[hp_cp_tsk_bin_warm_insert] '{0}','{1}','{2}','{3}','{4}'", id, warm_bin, bin_zb, warm_zb, warm_set));
                                                }
                                                break;
                                            case "大于等于":
                                                if (Convert.ToDouble(bin_zb) >= Convert.ToDouble(warm_zb))

...
namespace FileChange
{
    class ty_csv_helper03
    {
        ...
        public static void FileAnaly(string filePath, out List<string> dic_product_data, out List<string> dic_time_data, out List<string> dic_test_data, out Dictionary<string, TestData> dic_test_info)
        {
            ...
            {
                string str = ListTxt[i];
                if (str.Contains("Serial#,Bin#"))
                {
                    startIndex = i+1;
                }
            }
            ...
            int c = 0;
            for (int i = startIndex; i < ListTxt.Count; i++)
            {
                string str = ListTxt[i];
                ListTxt[i] = "1," + str;
            }
            ...
            foreach (var item in ListTxt)
            {
                ...
                if (item.ToString().Contains("SBin["))
                {
                    set_poist = 2;
                }
                ...
                switch (set_poist)
                {
                    ...
                    case 3:
                        ...
                        #endregion
                        //TODO  获取x和y坐标，但是不同的csv会变化
                        string x = item.ToString().Split(',')[4].ToString();
                        string y = item.ToString().Split(',')[5].ToString();
                        string SOFT_BIN = item.ToString().Split(',')[2].ToString();
                        string T_TIME = "";

...
namespace txt2tma
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                BatchConvert(Directory.GetCurrentDirectory());
            }
        }

        static void ConvertFile(string txtPath)
        {
            var (waferInfo, dataRows, rowCount, colCount) = ParseTxt(txtPath);
            if (dataRows.Count == 0)
            {
                Console.WriteLine("未找到RowData数据");
                return;
            }
            ...
            Console.WriteLine($"转换完成: {txtPath} -> {tmaPath}");
            Console.WriteLine($"数据行数: {dataRows.Count}, 每行数据数: {dataRows[0].Length}");
        }

        static void BatchConvert(string folderPath)
        {
            string[] txtFiles = Directory.GetFiles(folderPath, "*.txt");
            if (txtFiles.Length == 0)
            {
                Console.WriteLine("文件夹中没有txt文件");
                return;
            }
            ...
            foreach (string txtPath in txtFiles)
            {
                ...
                {
                    Console.WriteLine($"转换失败 {Path.GetFileName(txtPath)}: {ex.Message}");
                }
            }

            Console.WriteLine($"完成: {count}/{txtFiles.Length}");
        }

        static (Dictionary<string, string> waferInfo, List<string[]> dataRows, int rowCount, int colCount) ParseTxt(string txtPath)
        {
            ...
            foreach (string line in File.ReadAllLines(txtPath, Encoding.UTF8))
            {
                ...
                else
                {
                    ...
                    if (lineTrim.StartsWith("ROWCT:") && int.TryParse(lineTrim.Substring(6).Trim(), out int rc)) rowCount = rc;
                    if (lineTrim.StartsWith("COLCT:") && int.TryParse(lineTrim.Substring(6).Trim(), out int cc)) colCount = cc;
                }
            }
            return (waferInfo, dataRows, rowCount, colCount);
        }

        static List<string> ConvertToTma(Dictionary<string, string> waferInfo, List<string[]> dataRows, int rowCount, int colCount)
        {
            ...
            for (int i = 1; i <= colCount; i++)
                sb.Append(i.ToString().PadLeft(xDigits));
        ...
        static string Get(Dictionary<string, string> dict, string key) => dict.ContainsKey(key) ? dict[key] : "";
    }
}

...
namespace DataToExcel
{
    ...
    public class DieMatrix
    {
        ...
        public InkRuleResult ApplyInkRule(IInkRule rule, Dictionary<string, object> parameters)
        {
            ...
            return rule.Apply(this, parameters);
        }
        ...
        public List<Tuple<int, int>> PreviewInkResult(IInkRule rule, Dictionary<string, object> parameters)
        {
            ...
            return rule.Preview(this, parameters);
        }

        /// <summary>
        /// 获取所有可能被INK的Die列表（预览功能）
        /// </summary>
        /// <param name="rule">INK规则</param>
        /// <param name="parameters">参数</param>
        /// <returns>DieData列表</returns>
        public List<DieData> GetInkCandidates(IInkRule rule, Dictionary<string, object> parameters)
        {
            var coordinates = PreviewInkResult(rule, parameters);
            var candidates = new List<DieData>();

            foreach (var coord in coordinates)
            {
                candidates.Add(this[coord.Item1, coord.Item2]);
            }
        ...
        public Dictionary<int, int> GetBinDistribution()
        {
            ...
            {
                int bin = die.Bin;
                if (!distribution.ContainsKey(bin))
                    distribution[bin] = 0;
                distribution[bin]++;
            }
        ...
        /// 统计指定属性的Die数量
        /// </summary>
        /// <param name="attribute">Die属性</param>
        /// <returns>数量</returns>
        public int CountByAttribute(DieCategory attribute)
        {
            ...
            foreach (DieData die in this._items)
            {
                if ((die.Attribute & attribute) == attribute)
                    count++;
            }
            return count;
        }

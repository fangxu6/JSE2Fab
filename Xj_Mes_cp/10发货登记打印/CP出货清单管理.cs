...
namespace Xj_Mes_cp
{
    public partial class CP出货清单管理 : DockContent
    {
        public CP出货清单管理()
        {
            InitializeComponent();
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            string cu_name = this.pwtSearchBox1.Text;
            string address = this.pwtSearchBox2.Text;

            string send_code = this.textBox1.Text;
            ...
            string sql = string.Format("[dbo].[hp_1222_cp_send_print_info_send_print_select] '{0}','{1}','{2}','{3}','{4}','{5}'", cu_name, address, is_date, dat1, dat2, send_code);
        ...
        private void buttonX2_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < this.pwtDataGridView1.Rows.Count; i++)
            {
                if ((Convert.ToBoolean(pwtDataGridView1.Rows[i].Cells[0].Value) == true))
                {
                   // MessageBox.Show(pwtDataGridView1.Rows[i].Cells["序号"].Value.ToString());
                      ...
                      cus_address = this.pwtDataGridView1.Rows[i].Cells["出货地址"].Value.ToString();
            ...
            for (int i = 0; i < this.pwtDataGridView1.Rows.Count; i++)
            {
                if ((Convert.ToBoolean(pwtDataGridView1.Rows[i].Cells[0].Value) == true))
                {
                    ...
                    cus_address_use = this.pwtDataGridView1.Rows[i].Cells["出货地址"].Value.ToString();
                    string 测试程序 = this.pwtDataGridView1.Rows[i].Cells["测试程序"].Value.ToString();
                    string 箱数 = this.pwtDataGridView1.Rows[i].Cells["箱数"].Value.ToString();
                    string product_name = this.pwtDataGridView1.Rows[i].Cells["product_name"].Value.ToString();
                    string lot = this.pwtDataGridView1.Rows[i].Cells["lot"].Value.ToString();
                    string wafer_qty = this.pwtDataGridView1.Rows[i].Cells["wafer_qty"].Value.ToString();
                    string wafer_id = this.pwtDataGridView1.Rows[i].Cells["wafer_id"].Value.ToString();
                    string good_dies = this.pwtDataGridView1.Rows[i].Cells["good_dies"].Value.ToString();
                    string Mode = this.pwtDataGridView1.Rows[i].Cells["Mode"].Value.ToString();

                    string 补充备注说明 = this.pwtDataGridView1.Rows[i].Cells["补充备注说明"].Value.ToString();

                    remark += 补充备注说明;

                    try
                    {
                        box_number += int.Parse(箱数);
                    }
            ...
            string table_total = string.Format(table_down, box_number, pcs_number);
            ...
            for (int i = 0; i < this.pwtDataGridView1.Rows.Count; i++)
            {
                if ((Convert.ToBoolean(pwtDataGridView1.Rows[i].Cells[0].Value) == true))
                {
                    ...
                    ex.Exe_Data("[dbo].[hp_1222_cp_send_print_info_update_state_print_code_update]   '" + 序号 + "','已打印','" + send_code + "','" + base_info.user_code + "'");

                    this.pwtDataGridView1.Rows[i].Cells["出货打印状态"].Value = "已打印";
                    this.pwtDataGridView1.Rows[i].Cells["出货单号"].Value = send_code;
                }
            }
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < this.pwtDataGridView1.Rows.Count; i++)
            {
                if ((Convert.ToBoolean(pwtDataGridView1.Rows[i].Cells[0].Value) == true))
                {
                    string code = this.pwtDataGridView1.Rows[i].Cells["出货单号"].Value.ToString();
            ...
            string info = "确定取消" + code_list.Count.ToString() + "个单号？\r\n";
            ...
            if (MessageBox.Show(info, "系统提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) != System.Windows.Forms.DialogResult.OK)
            {
                return;
            }
            ...
            foreach (var item in code_list)
            {
                ex.Exe_Data("[dbo].[hp_1222_cp_send_print_info_update_state_print_code_cancle_update]  '" + item + "','" + base_info.user_code + "'");
            }
            ...
            foreach (var item in code_list)
            {
                for (int i = 0; i < this.pwtDataGridView1.Rows.Count; i++)
                {
                    ...
                    if (code==item)
                    {
                        ...
                        this.pwtDataGridView1.Rows[i].Cells["出货单号"].Value = "";
                    }
        ...
        private void CP出货清单管理_Load(object sender, EventArgs e)
        {

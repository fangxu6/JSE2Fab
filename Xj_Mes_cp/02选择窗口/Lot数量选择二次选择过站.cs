...
namespace Xj_Mes_cp
{
    public partial class Lot数量选择二次选择过站 : Form
    {
        public Lot数量选择二次选择过站(string only_lot,string post_list)
        {
            my_only_lot = only_lot;
        ...
        private string my_only_lot = "";

        private string my_post_list = "";
        ...
        private void LoadLotNo()
        {
            ...
            //DataTable dt_dongjie = ex.Get_Data(" [dbo].[hp_cp_hold_process_info_get_hold_post_select]   '" + my_only_lot + "'");
            ////处理片号

            //for (int d = 0; d < dt_dongjie.Rows.Count; d++)
            //{
            //    string dongjie_post = dt_dongjie.Rows[d]["位号"].ToString();

            //    for (int i = 0; i < h; i++)
            //    {
            //        for (int j = 0; j < w; j++)
            //        {
            //            if (!dongjie_post.Contains(this.dataGridView1.Rows[i].Cells[j].Value.ToString()))
            //            {
            //                this.dataGridView1.Rows[i].Cells[j].Style.BackColor = Color.Yellow;
            //                this.dataGridView1.Rows[i].Cells[j].Value = "冻结";
            //            }
            //        }
            //    }
            //}
            ...
            for (int i = 0; i < h; i++)
            {
                for (int j = 0; j < w; j++)
                {
                    if (!my_post_list.Contains(this.dataGridView1.Rows[i].Cells[j].Value.ToString()))
                    {
                        this.dataGridView1.Rows[i].Cells[j].Style.BackColor = Color.DarkGray;
                        this.dataGridView1.Rows[i].Cells[j].Value = "不可以选";
                    }
